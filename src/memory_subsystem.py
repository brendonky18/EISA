...
Memory subsystem
read: 
    takes in a 32 bit address
    returns value stored at address
write:
    takes in a 32 bit address and a value
    writes that value to the address

Cache:      implements write allocate vs no-write allocate
Cache Way: implements eviction for write back

Memory subsystem components:
    Cache:
        read:
            checks if the address is in the cache
            if yes: (cache hit)
        ...
        write:
            checks if the address is in the cache
            if yes: (cache hit)
                write the value to the cache way
            if no: (cache miss)
